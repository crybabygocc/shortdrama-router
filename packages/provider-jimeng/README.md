...
The standalone shortdrama-router executable does not require Node.js or npm. It downloads the platform-specific official CLI into shortdrama-router's application data directory, verifies its provider-pinned SHA-256 before installation and each execution, and uses only that absolute managed path without changing the user's PATH or shell profile. It never falls back to `~/.local/bin/dreamina`. Authorization is then started through the provider authorization API. `DREAMINA_CLI_PATH` remains an explicit developer override.
